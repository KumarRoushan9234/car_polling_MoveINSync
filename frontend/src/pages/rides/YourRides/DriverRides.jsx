  ...
  const getDriverRides = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/api/rides/driver_rides",
        {
          withCredentials: true,
        }
      );
      const driverRides = response?.data?.data || []; // Updated to fetch the correct response structure
      setRides(driverRides);
      setFilteredRides(driverRides);
    } catch (error) {
